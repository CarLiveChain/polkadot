...
impl backend::BlockImportOperation for BlockImportOperation {
	...
	fn set_storage<I: Iterator<Item=(Vec<u8>, Option<Vec<u8>>)>>(&mut self, _changes: I) -> error::Result<()> {
...
impl StateBackend for OnDemandState {
	...
	fn commit<I>(&mut self, _changes: I) where I: IntoIterator<Item=(Vec<u8>, Option<Vec<u8>>)> {
		// we do not store state => nothing to commit
	}
